...
class.sftp {
	...
	setOption = function (self, option, value)
		...
		elseif option == "timeout" then
			...
			return (libssh.ssh_options_set(self._session, 9, ffi.new("int", value)) >= 0)
		elseif option == "ssh1" then
			assert(type(value) == "boolean", "ssh1 must be boolean")
			return (libssh.ssh_options_set(self._session, 11, ffi.new("int", value and 1 or 0)) >= 0)
		elseif option == "ssh2" then
			assert(type(value) == "boolean", "ssh2 must be boolean")
			return (libssh.ssh_options_set(self._session, 12, ffi.new("int", value and 1 or 0)) >= 0)
		else
